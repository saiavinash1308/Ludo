...
public class MarkerDraggerScript : MonoBehaviour, IBeginDragHandler, IDragHandler
{
    ...
    public FixedJoystick joystick; // reference to the joystick
    ...
    public void OnDrag(PointerEventData eventData)
    {
        if (!BallControllerScript.instance.IsBallThrown)
        { // if the ball is not thrown, the marker can be moved
            newTouchPosition = eventData.position; // update the current position of the touch

            // set the marker's position based on the drag value of the touch
            marker.transform.position = new Vector3(
                markerStartTouchPosition.x + (newTouchPosition.x - startTouchPosition.x) * scaleDownDragBy,
    ...
    {
        // Use joystick input for marker movement
        float moveX = joystick.Horizontal * scaleDownDragBy;
        float moveZ = joystick.Vertical * scaleDownDragBy;

        // Update the marker's position based on the joystick input
        marker.transform.position += new Vector3(moveX, 0, moveZ);

        ClampMarkerPosition();
    }

    private void ClampMarkerPosition()
    {
        // Clamp the marker's position within its reach bounds
        marker.transform.position = new Vector3(
            Mathf.Clamp(marker.transform.position.x, customX, boundaryPointX),
            marker.transform.position.y,
